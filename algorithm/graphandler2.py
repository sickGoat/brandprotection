...
class GraphHandler(object):
    ...
    def activate(self,node,campaign,time,shadowActivation):
        raise NotImplementedError;
    ...
    def MultiCampaign(self,GL,BL,startingTime,startIndex):
        ...
        while badIt.hasNext() or goodIt.hasNext():
                    ...
                    for nbr in NBRS:
                        if self.canTry(nbr,Campaign.Good):
                            if self.attempt(node,nbr):
                                self.activate(nbr,Campaign.Good,t);
                                goodIt.append(nbr);
            ...
            try:
                ...
                while count > 0:
                    node = badIt.next();
                    shadowActivation = not self.isActiveFor(node,Campaign.Bad);
                    NBRS = self.G.neighbors(node);
                    for nbr in NBRS:
                        if self.canTry(nbr,Campaign.Bad):
                            if self.attempt(node,nbr):
                                if self.isActiveFor(nbr,Campaign.Good) or not self.isActiveFor(node, Campaign.Bad):
                                    self.save(nbr);
                                else:
                                    self.activate(nbr,Campaign.Bad,t,shadowActivation);
                                badIt.append(nbr);
                    count -= 1;
    ...
    def alghoritm(self,SEED,delay,k,heuristic):
        ...
        for x in SEED: self.activate(x,Campaign.Bad,0);
        ...
        GL = heuristic(SEED,k);
        for x in GL: self.activate(x,Campaign.Good,delay);
...
class RealGraphHandler(GraphHandler):
    def isActive(self, node):
        if self.G.node[node].has_key(self.state):
            return self.G.node[node][self.state] != NodeState.Inactive.value;
        
        return False;
    
    def isActiveFor(self, node, campaign):
        if self.G.node[node].has_key(self.state):
            return self.G.node[node][self.state] == campaign.value;
    ...
    def canTry(self, node, campaign):
        ...
        if campaign == Campaign.Bad:
            return self.G.node[node][self.shadowState] == NodeState.Inactive.value;
        
        return False;
        
    def activate(self, node, campaign, time,shadowActivation=False):
        if shadowActivation:
            self.G.node[node][self.state] = NodeState.Inactive.value;
            ...
            self.G.node[node][self.time] = time;
            if not self.G.node[node].has_key(self.shadowState):
                if campaign == Campaign.Bad:
                    self.G.node[node][self.shadowState] = NodeState.Bad.value;
                else:
                    self.G.node[node][self.shadowState] = NodeState.Inactive.value;
    ...
    def getInfected(self, L):
        for x in L:
            if self.G.node[x][self.state] != NodeState.Bad.value:
                del L[L.index(x)];
        
        return L; 
    
    def isSaved(self,node):
        if not self.G.node[node].has_key(self.state):
            return False;
    
        return self.G.node[node][self.state] != Campaign.Bad.value and self.G.node[node][self.shadowState] == Campaign.Bad.value;
